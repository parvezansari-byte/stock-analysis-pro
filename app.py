...
def make_gauge(value):
    ...
    fig.update_layout(height=265, margin=dict(l=10, r=10, t=34, b=4), paper_bgcolor="rgba(0,0,0,0)", font_color="white")
    return fig
...
def make_portfolio_risk_gauge(value):
    ...
    fig.update_layout(height=280, margin=dict(l=10, r=10, t=36, b=6), paper_bgcolor="rgba(0,0,0,0)")
    return fig
...
def build_stock_pdf(symbol, last_close, change_pct, ai_action, conviction_score, score, rsi, entry, stop_loss, target, qty, position_value, fund_verdict='N/A', tech_verdict='N/A', overall_ratio_score='N/A', fund_summary='N/A', tech_summary='N/A', overall_summary='N/A'):
    ...
    ratio_table = [
        ["Ratio Interpretation Engine", "Verdict / Score"],
    ...
    story.append(pdf_table(ratio_table, col_widths=[70 * mm, 100 * mm], header_bg="#1D4ED8"))
    story.append(Spacer(1, 6))
    ...
    story.append(pdf_table(chip_table, col_widths=[60 * mm, 110 * mm], header_bg="#0D9488"))
    story.append(Spacer(1, 6))
...
st.markdown("<div class='panel' style='padding:10px 14px;margin-top:4px;'><div style='display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px;'><div><span class='rank-badge'>Executive Polish</span> <span class='exec-chip'>Institutional Desk Finish</span> <span class='exec-chip'>Cloud Safe</span></div><div style='color:#93c5fd;font-weight:800;font-size:0.78rem;'>Sharper cards • cleaner tables • premium terminal feel</div></div></div>", unsafe_allow_html=True)
...
if not st.session_state.scan_df.empty:
    st.markdown("<div class='watchlist-matrix'><div class='panel-title'>Watchlist Decision Matrix</div><div class='subtle-divider'></div><div style='display:flex;gap:8px;flex-wrap:wrap;margin-bottom:8px;'><span class='ribbon-chip'>Ranked Institutional Watchlist</span><span class='ribbon-chip'>AI Decision Layer</span><span class='ribbon-chip'>Scanner Priority</span></div></div>", unsafe_allow_html=True)
    ...
    scan_table = st.session_state.scan_df.head(15).style.set_properties(**{'background-color': 'rgba(15,23,42,0.55)', 'color': 'white', 'border-color': 'rgba(255,255,255,0.05)'})\
        .set_table_styles([
            {'selector':'thead th','props':[('background','linear-gradient(135deg, rgba(34,197,94,0.28), rgba(34,211,238,0.18))'),('color','white'),('font-weight','900')]},
            {'selector':'tbody tr:hover','props':[('background-color','rgba(34,197,94,0.06)')]}
        ])
...
if portfolio_analysis_df.empty:
    ...
    portfolio_table = portfolio_analysis_df.style.set_properties(**{'background-color': 'rgba(15,23,42,0.55)', 'color': 'white', 'border-color': 'rgba(255,255,255,0.05)'})\
        .set_table_styles([
            {'selector':'thead th','props':[('background','linear-gradient(135deg, rgba(124,58,237,0.30), rgba(34,211,238,0.18))'),('color','white'),('font-weight','900')]}
        ])
...
if compare_symbols:
    ...
    .set_table_styles([
        {'selector':'thead th','props':[('background','linear-gradient(135deg, rgba(59,130,246,0.28), rgba(139,92,246,0.18))'),('color','white'),('font-weight','900')]}
    ])
...
# PDF REPORT EXPORT
# -------------------------------------------------
st.markdown("<div class='pdf-section-card'><div class='panel-title'>PDF Report Export</div><div class='subtle-divider'></div><div style='display:flex;gap:8px;flex-wrap:wrap;margin-bottom:8px;'><span class='ribbon-chip'>Premium Institutional PDF</span><span class='ribbon-chip'>Trade Plan Ready</span><span class='ribbon-chip'>Client Presentation Safe</span></div></div>", unsafe_allow_html=True)

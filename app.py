...
SECTOR_MAP = {
    ...
    "ADANIPORTS.NS":"Infrastructure","ADANIENT.NS":"Infrastructure","ADANIGREEN.NS":"Infrastructure","ADANIPOWER.NS":"Infrastructure","DLF.NS":"Realty","LODHA.NS":"Realty","POWERGRID.NS":"Utilities","NTPC.NS":"Utilities","COALINDIA.NS":"Utilities","JSWENERGY.NS":"Utilities",
}
...
def get_live_index(symbol: str):
    ...
    last = float(data["Close"].iloc[-1]); prev = float(data["Close"].iloc[-2])
...
def compute_indicators(df: pd.DataFrame):
    ...
    tr = pd.concat([tr1, tr2, tr3], axis=1).max(axis=1); d["ATR14"] = tr.rolling(14).mean()
...
def beautiful_top_card(title, value, change, bg, inverse=False):
    if pd.isna(value):
        st.markdown(f"<div class='hero-card' style='background:{bg};'><div class='hero-title'>{title}</div><div class='hero-value' style='font-size:1.05rem;color:#94a3b8;'>Data unavailable</div></div>", unsafe_allow_html=True)
        return
    up = change >= 0
    color = '#ef4444' if (inverse and up) else '#22c55e' if (inverse and not up) else '#22c55e' if up else '#ef4444'
    arrow = '▲' if up else '▼'
    st.markdown(f"<div class='hero-card' style='background:{bg};'><div class='hero-title'>{title}</div><div class='hero-value'>{value:,.2f}</div><div class='hero-change' style='color:{color};'>{arrow} {change:+.2f}%</div></div>", unsafe_allow_html=True)


def make_gauge(value):
    fig = go.Figure(go.Indicator(mode="gauge+number", value=value, title={'text': "Conviction"}, gauge={'axis': {'range': [0, 100]}, 'bar': {'color': "#06b6d4"}, 'steps': [
        {'range': [0, 40], 'color': "rgba(239,68,68,0.35)"}, {'range': [40, 70], 'color': "rgba(245,158,11,0.35)"}, {'range': [70, 100], 'color': "rgba(34,197,94,0.35)"}, ]}))
...
# -------------------------------------------------
# TOP RIBBON + BEAUTIFUL TOP CARDS
# -------------------------------------------------
...
allowed_risk = capital * (risk_pct / 100); qty = max(int(allowed_risk // risk_per_share), 0)
target = entry + (risk_per_share * rr_ratio); position_value = qty * entry
...
with m4: metric_box("ATR (14)", f"{atr:.2f}", "Volatility gauge", positive=None)
with m5:
...
if show_technical_ratio:
    ...
    tech_df = pd.DataFrame([
        ...
        {"Metric": "MACD", "Value": round(last['MACD'], 2)}, {"Metric": "MACD Signal", "Value": round(last['MACD_SIGNAL'], 2)}, {"Metric": "MACD Histogram", "Value": round(last['MACD_HIST'], 2)}, {"Metric": "ATR (14)", "Value": round(last['ATR14'], 2)},
    ])
...
with t1:
    if isinstance(bs, pd.DataFrame) and not bs.empty: st.dataframe((bs.iloc[:, :4].fillna(0) / 1e7).round(2), use_container_width=True)
    else: st.info("Balance sheet not available for this symbol.")
with t2:
    if isinstance(fin, pd.DataFrame) and not fin.empty: st.dataframe((fin.iloc[:, :4].fillna(0) / 1e7).round(2), use_container_width=True)
    else: st.info("Financials not available for this symbol.")
with t3:
    if isinstance(cf, pd.DataFrame) and not cf.empty: st.dataframe((cf.iloc[:, :4].fillna(0) / 1e7).round(2), use_container_width=True)
